...
import tentacles.Trading.Mode.market_cipher.settings as cipher_settings
...
class CipherTradeExecution:
    ...
    async def trade_live_candle(
        ...
        basic_utilities.end_measure_time(
            s_time,
            f" MarketCipher {symbol} - " "trading eventual singals",
        )
    ...
    def _cache_backtesting_signals(
        ...
        has_exit_signals: bool = len(exit_short_trades) or len(exit_long_trades)
        if has_exit_signals:
            (
                candle_times,
                start_short_trades,
        ...
        if self.trading_mode.order_settings.uses_managed_order:
            if self.trading_mode.order_settings.enable_long_orders:
                ...
                await basic_keywords.user_input(
                    ...
                    # },
                    parent_input_name=cipher_settings.ORDER_SETTINGS_NAME,
                )
